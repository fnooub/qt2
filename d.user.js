...
// @version      0.1
// @match        *://*/*
// @require      https://cdn.jsdelivr.net/gh/fnooub/qt2@main/qt3.js
// @grant        none
// ==/UserScript==
...
(async function () {
    ...
    // Hàm sửa đổi nội dung trang với các từ đặc biệt
    ...
    // Hàm dịch nội dung của một node DOM
    ...
    // Kiểm tra và dịch nội dung trang nếu có tiếng Trung
    try {
        var isChinese = document.title.match(/[\u3400-\u9FBF]/);
        if (isChinese) {
            document.documentElement.innerHTML = fixText(document.documentElement.innerHTML);
            await translateNode(document.documentElement);
        } else {
