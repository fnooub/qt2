...
class Dictionary {
    constructor() {
        ...
        this.cachedTrie = new Map();        // Cache dữ liệu từ trie
    }
    ...
    async init() {
        if (this.cachedTrie.has('CachedTrie')) {
            this.root = this.cachedTrie.get('CachedTrie');
            console.log('đã cache trie');

            // Đảm bảo rằng loadChinesePhienAm cũng được gọi khi cây Trie chưa được lưu trong cachedTrie
            const loadChinesePhienAm = this.readDictionaryFile('https://laongu.github.io/ChinesePhienAmWords.txt', (key, value) => {
                this.phienAmDictionary.set(key, value);
            });
            await loadChinesePhienAm;
        } else {
            await this.loadDictionaries();
            this.cachedTrie.set('CachedTrie', this.root);
            console.log('chưa cache trie');
        }
    }
